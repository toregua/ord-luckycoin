...
use crate::lky20::errors::Error::LedgerError;
use crate::lky20::operation::{InscriptionOp, Operation};
use crate::lky20::params::{BIGDECIMAL_TEN, MAX_DECIMAL_WIDTH};
...
use crate::lky20::{
  ...
  InscribeTransferEvent, LKY20Error, Message, Mint, MintEvent, Num, Tick, TokenInfo, Transfer,
...
pub(super) struct Lky20Updater<'a, 'db, 'tx> {
  lky20_token_info: &'a mut Table<'db, 'tx, &'static str, &'static [u8]>,
  lky20_token_holder: &'a mut MultimapTable<'db, 'tx, &'static str, &'static str>,
  lky20_token_balance: &'a mut Table<'db, 'tx, &'static str, &'static [u8]>,
  lky20_inscribe_transfer: &'a mut Table<'db, 'tx, &'static [u8; 36], &'static [u8]>,
  lky20_transferable_log: &'a mut Table<'db, 'tx, &'static str, &'static [u8]>,
...
impl<'a, 'db, 'tx> Lky20Updater<'a, 'db, 'tx> {
  pub(super) fn new(
    lky20_token_info: &'a mut Table<'db, 'tx, &'static str, &'static [u8]>,
    lky20_token_holder: &'a mut MultimapTable<'db, 'tx, &'static str, &'static str>,
    lky20_token_balance: &'a mut Table<'db, 'tx, &'static str, &'static [u8]>,
  ...
  pub(crate) fn index_block(
    ...
    log::info!(
      "LKY20 Updater indexed block {} with {} messages in {} ms",
      context.blockheight,
      messages_size,
  ...
  pub fn resolve_message(
    ...
    for input in &tx.input {
      ...
      while let Some(operation) = operation_iter.peek() {
        ...
        // Parse LKY20 message through inscription operation.
        if let Some(msg) = Message::resolve(
          &mut self.lky20_inscribe_transfer,
          &new_inscriptions,
          operation,
        )? {
          messages.push(msg);
  ...
  fn process_deploy(
    ...
    deploy: Deploy,
  ) -> Result<Event, errors::Error<LKY20Error>> {
    // ignore inscribe inscription to coinbase.
    let to_script_key = msg.to.clone().ok_or(LKY20Error::InscribeToCoinbase)?;

    let tick = deploy.tick.parse::<Tick>()?;

    if let Some(stored_tick_info) = Self::get_token_info(self, &tick).map_err(|e| LedgerError(e))? {
      return Err(errors::Error::LKY20Error(LKY20Error::DuplicateTick(
    ...
    if dec > MAX_DECIMAL_WIDTH {
      return Err(errors::Error::LKY20Error(LKY20Error::DecimalsTooLarge(dec)));
    }
    let base = BIGDECIMAL_TEN.checked_powu(u64::from(dec))?;

    let supply = Num::from_str(&deploy.max_supply)?;

    if supply.sign() == Sign::NoSign || supply > lky20::params::MAXIMUM_SUPPLY.to_owned() {
      return Err(errors::Error::LKY20Error(LKY20Error::InvalidSupply(
        supply.to_string(),
      )));
    }
    ...
    if limit.sign() == Sign::NoSign || limit > lky20::params::MAXIMUM_SUPPLY.to_owned() {
      return Err(errors::Error::LKY20Error(LKY20Error::MintLimitOutOfRange(
  ...
  fn process_mint(
    ...
    let token_info = Self::get_token_info(self, &tick)
      ...
      .ok_or(LKY20Error::TickNotFound(tick.to_string()))?;
    ...
    if amt.scale() > i64::from(token_info.decimal) {
      return Err(errors::Error::LKY20Error(LKY20Error::AmountOverflow(
        amt.to_string(),
      )));
    }
    ...
    if amt.sign() == Sign::NoSign {
      return Err(errors::Error::LKY20Error(LKY20Error::InvalidZeroAmount));
    }
    if amt > Into::<Num>::into(token_info.limit_per_mint) {
      return Err(errors::Error::LKY20Error(LKY20Error::AmountExceedLimit(
        amt.to_string(),
      )));
    }
    ...
    if minted >= supply {
      return Err(errors::Error::LKY20Error(LKY20Error::TickMinted(
  ...
  fn process_inscribe_transfer(
    ...
    if available < amt {
      return Err(errors::Error::LKY20Error(LKY20Error::InsufficientBalance(
        available.to_string(),
        amt.to_string(),
      )));
    }
  ...
  fn process_transfer(
    ...
    let mut transferable = Self::get_transferable_by_id(self, &msg.from, &msg.inscription_id)
      .map_err(|e| LedgerError(e))?
      .ok_or(LKY20Error::TransferableNotFound(msg.inscription_id))?;
    let amt = Into::<Num>::into(transferable.amount);

    if transferable.owner != msg.from {
      return Err(errors::Error::LKY20Error(
        LKY20Error::TransferableOwnerNotMatch(msg.inscription_id),
      ));
    }
  ...
  fn insert_transferable(
    ...
    self.lky20_transferable_log.insert(
  ...
  fn insert_inscribe_transfer_inscription(
    ...
    self.lky20_inscribe_transfer.insert(
  ...
  fn update_token_balance(
    ...
    self.lky20_token_balance.insert(
  ...
  fn remove_token_holder(
    ...
    self.lky20_token_holder.remove(
      tick.to_lowercase().hex().as_str(),
  ...
  fn insert_token_holder(&mut self, script_key: &ScriptKey, tick: Tick) -> Result<(), redb::Error> {
    self.lky20_token_holder.insert(
      tick.to_lowercase().hex().as_str(),
      script_key.to_string().as_str(),
    )?;
    Ok(())
