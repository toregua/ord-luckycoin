...
impl Lune {
  ...
  pub(crate) fn minimum_at_height(chain: Chain, height: Height) -> Self {
    ...
    let start = chain.first_lune_height();

    let end = start + SUBSIDY_HALVING_INTERVAL_10X;

    if offset < start {
      return Lune(Self::STEPS[12]);
    }
    ...
    Lune(start - ((start - end) * remainder / u128::from(INTERVAL)))
  }
  ...
  pub(crate) fn reserved(n: u128) -> Self {
    Lune(RESERVED.checked_add(n).unwrap())
  }
}

impl Serialize for Lune {
...
impl<'de> Deserialize<'de> for Lune {
  ...
  fn from_str(s: &str) -> crate::Result<Self> {
    ...
    for (i, c) in s.chars().enumerate() {
      ...
      match c {
        ...
        _ => bail!("invalid character in lune name: {c}"),
      }
    }
    Ok(Lune(x))
  }
...
mod tests {
  ...
  fn round_trip() {
    fn case(n: u128, s: &str) {
      assert_eq!(Lune(n).to_string(), s);
      assert_eq!(s.parse::<Lune>().unwrap(), Lune(n));
    }
  ...
  fn from_str_out_of_range() {
    "BCGDENLQRQWDSLRUGSNLBTMFIJAW".parse::<Lune>().unwrap_err();
  }
  ...
  fn mainnet_minimum_at_height() {
    ...
    fn case(height: u32, minimum: &str) {
      assert_eq!(
        Lune::minimum_at_height(Chain::Mainnet, Height(height)).to_string(),
        minimum,
      );
    }
  ...
  fn minimum_at_height() {
    ...
    fn case(chain: Chain, height: u32, minimum: &str) {
      assert_eq!(
        Lune::minimum_at_height(chain, Height(height as u64)).to_string(),
        minimum,
      );
    }
  ...
  fn serde() {
    let lune = Lune(0);
    let json = "\"A\"";
    assert_eq!(serde_json::to_string(&lune).unwrap(), json);
    assert_eq!(serde_json::from_str::<Lune>(json).unwrap(), lune);
  }
  ...
  fn reserved() {
    assert_eq!(
      RESERVED,
      "AAAAAAAAAAAAAAAAAAAAAAAAAAA".parse::<Lune>().unwrap().0,
    );

    assert_eq!(Lune::reserved(0), Lune(RESERVED));
    assert_eq!(Lune::reserved(1), Lune(RESERVED + 1));
  }

  #[test]
  fn is_reserved() {
    #[track_caller]
    fn case(lune: &str, reserved: bool) {
      assert_eq!(lune.parse::<Lune>().unwrap().is_reserved(), reserved);
    }
  ...
  fn steps() {
    for i in 0.. {
      match "A".repeat(i + 1).parse::<Lune>() {
        Ok(lune) => assert_eq!(Lune(Lune::STEPS[i]), lune),
        Err(_) => {
          assert_eq!(Lune::STEPS.len(), i);
          break;
        }
      }
    }
