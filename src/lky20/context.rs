...
use crate::lky20::operation::{deserialize_lky20_operation, Action, InscriptionOp, Operation};
use crate::lky20::transfer::Transfer;
use crate::lky20::TransferInfo;
...
impl Message {
  pub(crate) fn resolve<'a, 'db, 'tx>(
    lky20_inscribe_transfer: &'a mut Table<'db, 'tx, &'static InscriptionIdValue, &'static [u8]>,
    ...
    let lky20_operation = match op.action {
      Action::New { inscription: _ } if sat_in_outputs => {
        match deserialize_lky20_operation(
          new_inscriptions
          ...
          Ok(lky20_operation) => lky20_operation,
          _ => return Ok(None),
        }
      }
      // Transfered inscription operation.
      // Attempt to retrieve the `InscribeTransfer` Inscription information from the data store of LKY20.
      Action::Transfer => {
        match get_inscribe_transfer_inscription(lky20_inscribe_transfer, op.inscription_id) {
